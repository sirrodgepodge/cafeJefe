...
var addresses = [ "6147 Lakeside Dr #102, Reno, NV 89502" ], main = function() {
    ...
    $purchaseToggle.click(function() {
        ...
        $contactSubInside.text() !== contact[selected].text && ($contactSubLink.attr("href", contact[selected].link), 
        !!contact[selected].link === $contactSubLink.hasClass("disable-link") && $contactSubLink.toggleClass("disable-link"), 
        $contactSub.animate({
            opacity: 0
        }, 300, "swing", function() {
...
}, titleTop = Math.ceil($landing.outerHeight()), screenWidth = screen.width, contactTop = .92 * Math.ceil($contact.offset().top), downAnimReached = .395 * titleTop + 4.5, listeners = function() {
    ...
    landingScroll(), $(window).resize(function() {
        titleTop = Math.ceil($landing.outerHeight()), screenWidth = screen.width, downAnimReached = .395 * titleTop + 4.5, 
        landingScroll();
