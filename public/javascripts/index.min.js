...
var $purchaseToggle = $(".purchase-toggle"), $coffeeInd = $(".coffee-ind"), $buyBtn = $(".buy-btn"), $coffeePrice = $(".coffee-price"), $merchPrice = $(".merch-price"), $backToTop = $(".back-to-top, .title, .footer-logo"), $title = $(".title"), $contact = $(".contact"), $placeHolder = $(".place-holder"), $landingHead = $(".landing-head"), $landingNotHead = $(".landing-not-head"), $landingTogglers = $(".dream, .product"), $downAnim = $(".down-anim"), $contactImg = $(".contact-img"), $contactSubInside = $(".contact-sub-inside"), $contactSubLink = $contactSubInside.parent(), $contactSub = $contactSubLink.parent(), prices = {
    ...
    var purchTypeVal = $(".active-purch-type").attr("id"), inc = 0;
    $coffeePrice.each(function() {
        $(this).html("$" + prices[purchTypeVal][inc]), inc++;
    }), inc = 0, $merchPrice.each(function() {
        $(this).html("$" + prices.merch[inc]), inc++;
        ...
        }), purchTypeVal = $(this).attr("id"), $(".button-text").html(purchTypeVal.slice(0, 1).toUpperCase() + purchTypeVal.slice(1)), 
        inc = 0, $coffeePrice.each(function() {
            $(this).html("$" + prices[purchTypeVal][inc]), inc++;
        ...
        $contactSubInside.text() !== contactObj[selected].text && ($contactSubLink.attr("href", contactObj[selected].link), 
        !!contactObj[selected].link === $contactSubLink.hasClass("disable-link") && $contactSubLink.toggleClass("disable-link"), 
        $contactSub.animate({
            opacity: 0
        }, 300, "swing", function() {
...
}, titleTop = Math.ceil($title.offset().top), contactTop = .92 * Math.ceil($contact.offset().top), downAnimReached = .395 * titleTop + 4.5, listeners = function() {
    ...
    landingScroll(), $(window).resize(function() {
        titleTop = Math.ceil($title.offset().top), downAnimReached = .395 * titleTop + 4.5, 
        landingScroll();
