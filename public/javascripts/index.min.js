...
var addresses = [ "6147 Lakeside Dr #102, Reno, NV 89502" ], main = function() {
...
}, windowWidth = $(window).width(), titleTop = 768 > windowWidth ? 0 : Math.ceil($landing.outerHeight()), contactTop = .92 * Math.ceil($contact.offset().top), downAnimReached = .395 * titleTop + 4.5, listeners = function() {
    var pagePos = 0, titleFixed = !1, landingHeadDimmed = !1, topTextShowing = !0, contactPopped = !1, landingScroll = function() {
        pagePos = window.pageYOffset, 768 > windowWidth ? titleFixed || ($title.toggleClass("sticky"), 
        $placeHolder.toggleClass("no-show"), titleFixed = !titleFixed) : (pagePos >= titleTop && !titleFixed || titleTop > pagePos && titleFixed) && ($title.toggleClass("sticky"), 
    ...
    landingScroll(), $(window).resize(function() {
        windowWidth = $(window).width(), titleTop = 768 > windowWidth ? 0 : Math.ceil($landing.outerHeight()), 
        downAnimReached = .395 * titleTop + 4.5, contactTop = .92 * Math.ceil($contact.offset().top), 
        landingScroll();
